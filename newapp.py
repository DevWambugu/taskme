...
@main.route('/application.html', strict_slashes=False)
def application_page():
    return render_template('services.html')

@main.route('/registration.html', methods=['GET', 'POST'])
def add_register():
...
@main.route('/application2.html', methods=['GET', 'POST'])
def add_applicant():
    if request.method == 'POST':
...
@main.route("/getUserData", methods=['POST'])
def getUserData():
    if request.method == 'POST':
        cursor = mysql_connection.cursor(dictionary=True)
...
@main.route("/getAllServices", methods=['GET'])
def getAllServices():
    #cursor = 
...
@main.route("/addComments", methods=['POST'])
def addComments():
...
@main.route("/logoutUser", methods=['POST'])
def logoutUser():
    if request.method =='POST':
        cursor = mysql_connection.cursor(dictionary=True)
